...
public class Master {
    ...
    private static final String NUKE_KEY = "nuke";
    private static final int NUKE_ACTION_PERIOD = 900;
    private static final int NUKE_ANIMATION_PERIOD = 40;

    private static final int SUPER_ACTION_PERIOD = 900;
    private static final String SUPER_KEY = "superMiner";
    private static final int SUPER_ANIMATION_MIN = 50;
    private static final int SUPER_ANIMATION_MAX = 150;
    ...
    */
    ...
    public static int nukeX(){
        return rand.nextInt(32)+1;
    }
    public static int nukeY(){
        return rand.nextInt(32)+1;
    }
    public static Entity armageddon(WorldModel world, ImageStore imageStore){
        int start = nukeX();
        Point pos = new Point(start,0);
        if(world.isOccupied(pos)){
            world.removeEntityAt(pos);
        }
        Entity nuke = Factory.createNuke(NUKE_KEY,pos,imageStore.getImageList(NUKE_KEY),NUKE_ACTION_PERIOD,NUKE_ANIMATION_PERIOD);
        world.tryAddEntity(nuke);
        return nuke;
    }
    public static Entity hero(Point p,WorldModel world, ImageStore imageStore){

        if(world.isOccupied(p)){
            world.removeEntityAt(p);
        }
        Entity superMiner = Factory.createSuperMiner(SUPER_KEY ,
                p, SUPER_ACTION_PERIOD,
                SUPER_ANIMATION_MIN +
                        Factory.rand.nextInt(SUPER_ANIMATION_MAX - SUPER_ANIMATION_MIN),
                imageStore.getImageList(SUPER_KEY));
        world.tryAddEntity(superMiner);
        return superMiner;
    }
    static final Function<Point, Stream<Point>> CLICK_RADIUS =
            point ->
                    Stream.<Point>builder()
                            ...
                            .add(new Point(point.getX()-2, point.getY()-1))
                            .add(new Point(point.getX()-2, point.getY()+1))
                            .add(new Point(point.getX() +1, point.getY()-2))
                            .add(new Point(point.getX()+1, point.getY()+2))
                            .add(new Point(point.getX()-1, point.getY()-2))
                            .add(new Point(point.getX()-1, point.getY()+2))
                            ...
                            .add(new Point(point.getX()-2, point.getY()-2))
                            .add(new Point(point.getX()-2, point.getY()+2))
                            .add(new Point(point.getX() +2, point.getY()-3))
                            .add(new Point(point.getX()+2, point.getY()+3))
                            .add(new Point(point.getX()-2, point.getY()-3))
                            .add(new Point(point.getX()-2, point.getY()+3))
